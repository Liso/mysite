function addToList(where, name, id, speed){
    var inputBoxEl   = document.getElementById(id);
    var itemText = name;
...
//    document.getElementById("addToBottomButton").disabled = true;
    ...
    moveIt(endingPos[0],endingPos[1], speed);
...
function deleteItem(){
    ...
    var invoker     = this;
    document.getElementById(this.pid).disabled = false;
...
function moveIt(finalX,finalY,speed){
    ...
    setTimeout("moveIt("+finalX+","+finalY+","+speed+")",30);
}
...
function addToMenu(e) {
    e.disabled = true;
    addToList('bottom', e.name, e.id, 10);
}
